...
export const ThreeJewelleryViewer: React.FC<Props> = ({
  ...
  return (
    <div className="relative w-full h-full min-h-[480px] select-none flex flex-col justify-between bg-gradient-to-b from-[#241A18]/5 via-transparent to-[#241A18]/5">
      {/* 3D WebGL Canvas */}
      <div
        ref={mountRef}
      ...
      <div className="absolute top-4 left-4 right-4 flex flex-wrap items-center justify-between gap-2 z-10 pointer-events-auto">
        {/* Ornament Category Pills */}
        <div className="flex items-center gap-1 bg-[#1F1615]/85 backdrop-blur-md p-1 rounded-2xl border border-white/10 shadow-xl">
          {ornamentTabs.map((t) => (
            <button
              key={t.id}
              type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold backdrop-blur-md transition-all ${
              isAutoRotating
      ...
      <div className="absolute bottom-22 left-4 right-4 flex items-center justify-between pointer-events-auto z-10">
        {/* Camera Angles */}
        <div className="flex items-center gap-1 bg-white/90 backdrop-blur-md p-1 rounded-xl border border-stone-200 shadow-lg">
          <button
            type="button"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-[#D4AF37] to-[#F3DE8A] text-[#2B090F] font-bold text-xs rounded-xl shadow-md hover:brightness-105 active:scale-95 transition-all"
        >
      ...
      {showControlsBar && (
        <div className="relative z-10 bg-white/95 backdrop-blur-md border-t border-[#D4AF37]/25 p-3 rounded-b-2xl shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
            {/* Precious Metal Selector */}
            <div>
              <div className="text-[11px] font-bold text-[#4A1017] uppercase tracking-wider mb-1">
                Gold & Metal
              </div>
                ...
                {(['gold24k', 'gold22k', 'rosegold', 'platinum'] as MetalType[]).map((m) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs rounded-lg border text-center font-medium transition-all ${
                        isSel
