      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-stone-200 bg-white p-4 space-y-3 shadow-lg animate-in slide-in-from-top duration-200">
          ...
          <div className="relative">
            <div className="w-full flex items-center gap-2 bg-stone-50 px-3 py-2 rounded-xl border border-stone-200 focus-within:border-[#4A1017]">
            ...
            {isSearchOpen && searchQuery.trim() && (
              <div className="mt-1 bg-white rounded-xl shadow-lg border border-stone-200 p-2 max-h-56 overflow-y-auto space-y-1">
                {filteredSearchResults.length > 0 ? (
                  filteredSearchResults.map((p) => (
                    <div
                      key={p.id}
                      ...
                      className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-stone-50 cursor-pointer"
                    >
                      <img
                        src={p.image}
                        alt={p.name}
                        className="w-8 h-8 object-cover rounded-md border border-stone-200"
                      />
                      <div className="flex-1 min-w-0 text-left">
                        <div className="text-xs font-semibold text-stone-900 truncate">
                          {p.name}
                        </div>
                        <div className="text-[10px] text-[#4A1017]">
                          ₹{p.price.toLocaleString('en-IN')}
                        </div>
                      </div>
                    </div>
